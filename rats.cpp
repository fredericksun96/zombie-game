...
int Rat::row() const
{
	//Returns the row the rat is on
	return m_row;
}

int Rat::col() const
{
	//Returns the column the rat is on
	return m_col;  
}

bool Rat::isDead() const
{
	// Return whether the Rat is dead
	if (m_dead)
		return true;
	else 
		return false;  
...
void Rat::move()
{
	...
	else if (m_poison == 1 && m_turnsSincePos % 2 == 0)
	{
		int random = 0;
		random = rand() % 100;
...
string Player::move(int dir)
{
	...
	if (dir == NORTH)
	{
		...
		if (m_arena->numberOfRatsAt(m_row, m_col) >= 1)
		{
...
{
	// Return the number of rats in the arena
	return m_nRats;
}
...
char intToChar(int i)		//RETURNS A INTEGER CHAR FROM AN INT, USED IN ARENA::DISPLAY
{
	if (i == 0)
		return '0';
	else if (i == 1)
...
void Arena::display(string msg) const
{
	...
	for (r = 1; r <= rows(); r++)
		for (c = 1; c <= cols(); c++)
			if (numberOfRatsAt(r, c) == 1)		//if there's one rat at a grid part, prints out 'R'
				displayGrid[r - 1][c - 1] = 'R';
			else if (numberOfRatsAt(r, c) > 1)		//if there's more than one, prints out the number there are
			{
				displayGrid[r - 1][c - 1] = intToChar(numberOfRatsAt(r, c));
			}
...
void Arena::moveRats()
{
	...
	for (int i = 0; i < m_nRats; i++)//If the rat that's dead isn't the last in the array, shift it
	{
		if (m_rats[i]->isDead())
		{
			delete m_rats[i];	//Deletes dynamically allocated storage for the dead rat
		...
		/*	else if (i == m_nRats - 1)	// if the rat that's dead is the last in the array
			{
				m_rats[m_nRats - 1] = nullptr;
				m_nRats--;
			}*/
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
	if ((a.numberOfRatsAt(r + 1, c) > 0 || a.numberOfRatsAt(r, c + 1) > 0 || a.numberOfRatsAt(r, c - 1) > 0) && r - 1 > 0)
	{
		bestDir = NORTH;
		return true;
	}
	if ((a.numberOfRatsAt(r - 1, c) > 0 || a.numberOfRatsAt(r+ 1, c) > 0 || a.numberOfRatsAt(r, c - 1) > 0) && c + 1 <a.cols())
	{
		bestDir = EAST;
		return true;
	}
	if ((a.numberOfRatsAt(r  - 1, c) > 0 || a.numberOfRatsAt(r, c + 1) > 0 || a.numberOfRatsAt(r, c - 1) > 0) && r + 1 < a.rows())
	{
		bestDir = SOUTH;
		return true;
	}
	...
	if (a.getCellStatus(r, c) == HAS_POISON && r - 1 > 0 && a.getCellStatus(r - 1, c) != HAS_POISON &&
		a.numberOfRatsAt(r - 2, c) == 0 && a.numberOfRatsAt(r - 1, c - 1) == 0 && a.numberOfRatsAt(r-1, c + 1) == 0)
	{
		bestDir = NORTH;
		return true;
	}
	else if (a.getCellStatus(r, c) == HAS_POISON && c + 1 <= a.cols() && a.getCellStatus(r, c+ 1) != HAS_POISON &&
		a.numberOfRatsAt(r, c+ 2) == 0 && a.numberOfRatsAt(r - 1, c + 1) == 0 && a.numberOfRatsAt(r + 1, c + 1) == 0)
	{
		bestDir = EAST;
		return true;
	}
	else if (a.getCellStatus(r, c) == HAS_POISON && r + 1 <= a.rows() && a.getCellStatus(r + 1, c) != HAS_POISON &&
		a.numberOfRatsAt(r + 2, c) == 0 && a.numberOfRatsAt(r + 1, c +1 )==0 && a.numberOfRatsAt(r+ 1, c - 1) == 0)
	{
		bestDir = SOUTH;
		return true;
	}
	else if (a.getCellStatus(r, c) == HAS_POISON && c-  1 > 0 &&
		a.numberOfRatsAt(r, c -2) ==0 && a.numberOfRatsAt(r + 1, c - 1) == 0 && a.numberOfRatsAt(r - 1, c - 1) ==0)
	{
		bestDir = WEST;
		return true;
